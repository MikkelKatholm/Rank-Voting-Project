...
./../compile.py rcv_matrix.mpc

while [ $v_val -le 100 ]; do

reps=10
i=0

touch "Outputs/voters_repetition/output-c${c_val}-s${s_val}-v${v_val}-f${f_val}.txt"

while [ $i -le $reps ]; do
echo "i = $i"
../Scripts/mascot.sh rcv_matrix -N $s_val -IF Player-Data/matrix >> "Outputs/voters_repetition/$v_val.txt"
i=$((i + 1))
done
v_val=$((v_val + 5))
done